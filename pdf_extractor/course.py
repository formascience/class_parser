...
class Course(BaseModel):
    ...
    # Academic structure metadata
    level: str  # Academic level (e.g., "L1", "L2")
    block: str  # Academic block (e.g., "BLOC_SANTE", "BLOC_TRANSVERSAL", "BLOC_DISCIPLINAIRE")
    semester: str  # Semester (e.g., "S1", "S2")
    subject: str  # Subject/UE (e.g., "CONSTITUTION_MATIERE", "MOLECULES_VIVANT")
    chapter: str  # Chapter (e.g., "CHAPITRE_1", "CHAPITRE_2")
    title: str  # Course title (e.g., "Introduction aux concepts fondamentaux")
    ...
    def print_outline(self) -> str:
        ...
        if not self.content:
            return f"No outline available for course: {self.title}"
        
        header = f"Course: {self.title}\n"
        header += f"Level: {self.level}\n"
        header += f"Block: {self.block}\n"
        header += f"Semester: {self.semester}\n"
        header += f"Subject: {self.subject}\n"
        header += f"Chapter: {self.chapter}\n"
        if self.year:
    ...
    def print_content(self, max_chars_per_line: int = 80) -> str:
        ...
        if not self.content:
            return f"No content available for course: {self.title}"
    ...
    def export_to_docx(self, output_path: str = None, include_course_info: bool = True) -> str:
        ...
        Args:
            output_path: Path where to save the DOCX file (default: "{course_title}.docx")
        ...
        """
        if not self.content:
            raise ValueError(f"No content available for course: {self.title}")
        ...
        if include_course_info:
            title_parts.append(f"({self.level} - {self.block} - {self.semester})")
            title_parts.append(f"{self.subject} - {self.chapter}")
            if self.year:
